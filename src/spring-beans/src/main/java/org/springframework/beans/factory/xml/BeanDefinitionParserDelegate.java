...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		if (beanDefinition != null) {
			...
			//封装成 BeanDefinitionHolder 对象，此对象的目的是增加一个aliases和beanName，通过beanName找bean（或者aliases找beanName,再找beanName）
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			...
			//解析bean中constructor-arg标签,关注最后封装的ConstructorArgumentValues和MutablePropertyValues 两个类生成的对象
			...
			if (isCandidateElement(node) && nodeNameEquals(node, REPLACED_METHOD_ELEMENT)) {
				...
				for (Element argTypeEle : argTypeEles) {
					...
					if (StringUtils.hasText(match)) {
						//因为函数重载，需要函数参数类型或者参数个数不同
						//存储多个参数，用于在replaceMethod中决定具体调用哪个Method
	...
	public BeanDefinitionHolder decorateIfRequired(
		...
		if (namespaceUri != null && !isDefaultNamespace(namespaceUri)) {
			NamespaceHandler handler = this.readerContext.getNamespaceHandlerResolver().resolve(namespaceUri);
			if (handler != null) {
				//调用NamespaceHandler处理类的decorate方法，开始具体的装饰过程，并返回装饰完的对象
